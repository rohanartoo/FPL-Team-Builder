...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<"list" | "viz" | "schedule" | "team">("list");
  ...
  const getNextFixtures = (teamId: number, count: number = 5, offset: number = 0) => {
    const upcoming = fixtures.filter(f => !f.finished && (f.team_h === teamId || f.team_a === teamId))
    ...
    return upcoming.map(f => {
      ...
      return {
        opponent: getTeamShortName(opponentId),
        difficulty,
        isHome,
        event: f.event
      };
  ...
  const fetchMyTeam = async (id: string) => {
    ...
    try {
      ...
      const enrichedSquad = picksData.picks.map((pick: any) => {
        ...
        const realForm = playerSummaries[player.id] ? metrics.points : parseFloat(player.form);
        ...
        return {
          ...
          fixtureEase,
          realForm,
          valueScore: parseFloat((realForm * fixtureEase).toFixed(2))
        };
      });
    ...
    const suggestions = weakLinks.map(outPlayer => {
      ...
      const betterOptions = players
        .filter(p => 
          ...
          p.id !== outPlayer.id &&
          p.now_cost <= budget &&
          !mySquad.some(s => s.id === p.id) // Not already in squad
        )
        .map(p => {
          const metrics = calculateLast5Metrics(p.id);
          const fixtureEase = calculateFixtureEase(p.team);
          const realForm = playerSummaries[p.id] ? metrics.points : parseFloat(p.form);
          return {
            ...p,
            fixtureEase,
            realForm,
  ...
  // Automatically fetch summaries for top 30 players to populate viz
  useEffect(() => {
    if (players.length > 0 && !isFetchingSummaries) {
      const topPlayers = players
        .filter(p => parseFloat(p.form) > 3)
  ...
  const vizData = useMemo(() => {
    return processedPlayers
      .filter(p => playerSummaries[p.id]) // Only plot players with fetched summaries
      .map(p => ({
        name: p.web_name,
        form: p.realForm,
        ease: p.fixtureEase,
      ...
      <main className="max-w-7xl mx-auto">
        {activeTab === "list" ? (
          <div className="border-t border-[#141414]">
            {/* Table Header */}
            <div className="hidden md:grid grid-cols-[40px_2fr_0.8fr_0.8fr_0.8fr_0.8fr_0.5fr_0.5fr_0.5fr_0.5fr_1.5fr] p-4 border-b border-[#141414] font-serif italic text-xs opacity-50 uppercase tracking-widest text-center">
            ...
            <div className="divide-y divide-[#141414]">
              {processedPlayers.map((player, index) => {
                ...
                const valueScore = parseFloat((player.realForm * player.fixtureEase).toFixed(2));
                ...
                return (
                  <div key={player.id} className="group">
                    <div 
                      ...
                      className={`grid grid-cols-1 md:grid-cols-[40px_2fr_0.8fr_0.8fr_0.8fr_0.8fr_0.5fr_0.5fr_0.5fr_0.5fr_1.5fr] p-4 items-center cursor-pointer transition-all text-center
                        ...
                        {upcoming.map((f, i) => (
                          <div 
                            ...
                            className={`w-8 h-8 flex items-center justify-center font-mono text-[10px] border border-current
                              ${f.difficulty <= 2 ? 'bg-emerald-500/20' : f.difficulty >= 4 ? 'bg-rose-500/20' : ''}`}
                            title={`${f.opponent} (${f.isHome ? 'H' : 'A'}) - FDR: ${f.difficulty}`}
                          >
                            {f.opponent}
                          </div>
                        ))}
                      ...
                      <div key={i} className="grid grid-cols-1 md:grid-cols-[1fr_auto_1.5fr] gap-8 items-center bg-[#141414]/5 p-6 border border-[#141414]/10">
                        ...
                        <div className="space-y-3">
                          ...
                          {suggestion.options.length === 0 ? (
                            ...
                            suggestion.options.map((opt: any, j: number) => (
